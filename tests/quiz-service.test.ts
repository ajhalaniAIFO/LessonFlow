...
describe("quiz-service", () => {
  ...
  it("lists quiz attempts in reverse chronological order", () => {
    const db = getDatabase();
    const now = Date.now();
    ...
    const attempts = listQuizAttempts("lesson-1", "scene-1");

    expect(attempts).toHaveLength(2);
    expect(attempts[0]?.score.correct).toBe(1);
    expect(attempts[1]?.score.correct).toBe(0);
    expect(attempts[0]?.results[0]?.questionId).toBe("q1");
  });
});
